...
# Título da aplicação
st.title(":green[>> TRIGO]")

#Imagem Topo
st.image('https://i.imgur.com/a3ZQXWD.jpg', use_column_width='always')
...
with st.sidebar:
    ...
    trigo.page_link("pages/trigo.py", label="TRIGO", disabled=True)
...
def load_data():
    ...
    select 
        ...
        'PREÇO MÉDIO DO TRIGO CEPEA/ESALQ - PARANÁ' as ds_serie 
        from `insight_esalq.tb_precos_trigo` 
    where 
        cast(dt_preco as date format 'dd/mm/yyyy') > '2023-01-01' 
        and ds_serie = 'Trigo | PREÇO MÉDIO DO TRIGO CEPEA/ESALQ - PARANÁ                                                 '
...
#Fonte
st.write(":green[FONTE: PREÇO MÉDIO DO TRIGO CEPEA/ESALQ - PARANÁ]")
st.write(":green[NOTA: Trigo pão ou melhorador, preço por tonelada, mercado disponível, à vista (prazo descontado pela taxa NPR).]")
