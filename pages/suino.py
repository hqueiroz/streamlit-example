...
# Título da aplicação
st.title(":green[>> SUÍNO]")

#Imagem Topo
st.image('https://i.imgur.com/QwYBeDK.jpg', use_column_width='always')
...
def load_data():
    ...
    select 
        ...
        ds_produto, 
        vl_preco_a_vista as vl_preco, 
        'INDICADOR DO SUÍNO VIVO CEPEA/ESALQ (R$/kg)  - PR' as ds_serie 
        from `insight_esalq.tb_precos_suino` 
    where 
        cast(dt_preco as date format 'dd/mm/yyyy') > '2023-01-01' 
        and ds_serie = 'Suíno | INDICADOR DO SUÍNO VIVO CEPEA/ESALQ (R$/kg)  - PR                                           '
        and vl_preco_a_vista <> '-' 
    order by 
...
#Fonte
st.write(":green[FONTE: INDICADOR DO SUÍNO VIVO CEPEA/ESALQ (R$/kg)  - PR]")
st.write(":green[NOTA: Preço recebido pelo produtor R$/Kg sem ICMS]")
