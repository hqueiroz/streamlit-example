...
# Título da aplicação
st.title(":green[>> BEZERRO]")

#Imagem Topo
st.image('https://i.imgur.com/5HsT0vl.jpg', use_column_width='always')
...
with st.sidebar:
    ...
    bezerro.page_link("pages/bezerro.py", label="BEZERRO", disabled=True)
...
def load_data():
    ...
    select 
        ...
        'Bezerro - Média Estado de São Paulo' as ds_serie 
        from `insight_esalq.tb_precos_bezerro` 
    where 
        ds_serie = 'Bezerro | Bezerro - Média Estado de São Paulo                                                  '
...
#Fonte
st.write(":green[FONTE: Bezerro - Média Estado de São Paulo]")
st.write(":green[NOTA: R$/cabeça a partir de 4 de julho de 2012, o prazo de pagamento passou a ser descontado pela taxa CDI; antes, era usada a NPR]")
